...
(defn topo-load-via-cdb []
  (fn [e]
    ...
    (def plugin-type "topo_load_via_cdb")
...
;      )
    (process-state-ek1-mk2val
      ...
      plugin-type
      (fn [strm event regex-match]
        (let [domain-name (remove-from-end (get-substr-after-last-char (get regex-match 1) "/") "}")]
          (assoc strm
    ...
    (process-state-ek1-mk2val
      ...
      (fn [strm event regex-match]
        ...
        (let [concise-msg (apply str (now) ": VND '" (:domain_name strm) "' created successfully (took"
...
(defn topo-load-via-cdb-expired [& children]
  (fn [strm]
    ...
    (let [concise-msg (apply str (now) ": VND '" (:domain_name strm) "' creation failed."
