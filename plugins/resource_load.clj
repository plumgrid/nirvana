      ...
      (fn [event regex-match]
        (let [ip (get regex-match 1) cpu-load (parse-int (get regex-match 2)) mem-load (parse-int (get regex-match 3))]
          (if (and (> cpu-load 90) (<= cpu-load 97))
            (do
              (def concise-msg (apply str (now) ": WARN::" (:time event) " CPU load of director node " (:my_host event)
          ...
          (if (> cpu-load 97)
            (do
              (def concise-msg (apply str (now) ": ERROR::" (:time event) " CPU load of director node " (:my_host event)
                                 " IP:" ip " is at " cpu-load "."
                                 )
                )
          ...
          (if (and (> mem-load 90) (<= mem-load 97))
            (do
              (def concise-msg (apply str (now) ": WARN::" (:time event) " Memory load of director node " (:my_host event)
          ...
          (if (> mem-load 97)
            (do
              (def concise-msg (apply str (now) ": ERROR::" (:time event) " Memory load of director node " (:my_host event)
                                 " IP:" ip " is at " mem-load "."
                                 )
                )
              (file-write error-log-location [concise-msg "\n"])
